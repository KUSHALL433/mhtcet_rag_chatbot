...
def call_llm(state: MessagesState) -> MessagesState:
    ...
    if messages and isinstance(messages[-1],ToolMessage):
        ...
        system_prompt = f"""
            ...
            If you need to look up some information before asking a follow up question, you are allowed to do that! You can remeber user specified information like (name).
...
def delete_user_session(session_id: str):
    """Deletes all data related to a specific user session."""
    try:
        for checkpoint in checkpointer.list(None):
            config = checkpoint.config.get('configurable', {})
            if config.get('thread_id') == session_id:
                checkpointer.delete(checkpoint.config)
        print(f"✅ Deleted data for session: {session_id}")
